...
### 6.1. Кто разряжает гарантию полноты

Дописано 2026-08-01, OWNER: slasten. REPLACES: NONE.

```text
правила зоны        здесь
исполнение долива   модуль деки
запуск              тот, кто опустошил слот
```

Долив `deck -> targets[i]` есть операция `ВЗЯТЬ` модуля деки с назначением «цели»: `DECK_MODULE_LAW §5.1`. Целевая зона в этой операции — **назначение**, а не исполнитель.

Отсюда следствие, ради которого раздел и написан:

```text
модуль деки не может долить  ->  «выдать нечем»
```

То есть исчерпание деки покрывает целевую зону тем же условием, что манифест и латент. Отдельного правила для целей не требуется.

Целевая зона **не является модулем**: собственных операций у неё нет. Всё, что с ней происходит, делают другие по правилам, записанным здесь.
