...
Дописано 2026-08-01, OWNER: slasten. REPLACES: NONE.

```text
владеет   МИНОРНАЯ МАШИНА
```

Основание — замер, а не предпочтение:

```text
пишет      одно место: оператор ☱ RUNTIME
читают     легальность хода (runtime_granted_operators -> logic_available),
           и поверхности
```

Зона обслуживает **джокер**, то есть решение о легальности хода, а оно принадлежит минорной машине целиком.

```text
внешний писатель ПРОСИТ, а не кладёт
```

`REQUIEM` — единственный козырь, устанавливающий карту в `runtime`. Он не пишет туда сам: он обращается к минорной машине, как козыри обращаются к модулю деки за картой.

Форма та же, что уже применена дважды: у латента (`MINOR_MACHINE_LAW §3`) и у целевой зоны (`TARGET_ZONE_LAW §6.1`) — правила живут в законе зоны, исполнение у названного владельца.
