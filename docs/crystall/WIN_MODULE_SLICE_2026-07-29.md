...
Открытое: должен ли козырь в целевой зоне быть **раскрытым**, чтобы участвовать в компиляции. `WIN_CHECK_LAW §3` говорит «contains 3 trumps» и про раскрытие молчит, `TARGET_ZONE_LAW §7` делает известного козыря открытым. Срез реализует **по присутствию, без требования раскрытия**; если задумано иначе — правка в одну строку `compiled_sequence`.
...
5. предикат ENOUGH и claim (§6), проверки 6-8
6. проверки 11-12
```
