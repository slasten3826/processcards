...
RUNTIME-поле — графитовое (`{0.18, 0.20, 0.24}`), очень тёмное.
На затемнённом поле глиф RUNTIME того же цвета будет нечитаем.

Решение: триграмма RUNTIME (☱) рисуется контуром (`"line"`), а не заливкой (`"fill"`).
...
### src/glyphs.lua
- RUNTIME — поменять `"fill"` на `"line"` в draw_trigram
